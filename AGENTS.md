...
- Former `mempool` callers (graph, entry, file, pel_group, tent_tree, tent_list, tupid_list) now use `malloc`/`free` directly.
- jp_alloc's design: **magazine-based global freelist** (memcpy refill/flush) + **3-epoch-ring EBR** for ABA-freedom + **per-thread TLS cache** (N=32 per size class) + **binary buddy split** (pool count 24, 8MB max) + **madvise(MADV_DONTNEED)** for large blocks (≥4KB) at EBR drain time. Magazines are allocated from the pool system (pool 8 = 256B blocks) and recycled via a CAS-based free-list — no static arrays, no mmap for magazines. Thread teardown is leak-free: a `pthread_key` destructor flushes the exiting thread's caches and per-epoch retired batches into a global limbo, which live threads drain lazily.
...
- The standalone benchmark/correctness test `src/jp_alloc/jp_alloc_bench.c` (compiled with `-DJP_ALLOC_BENCH` and `jp_alloc.c` into a `main()` binary) spawns 300 pthreads doing alloc/free churn and reports ops/sec, p50/p99 latency, and peak RSS. The shell wrapper `test/t5800-jp-alloc-threads.sh` builds release + `-DJP_ALLOC_DEBUG` + 300-thread-stress variants; the debug variant aborts on ABA / double-free / corruption, so reaching "no ABA" output is the regression gate.
