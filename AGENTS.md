...
- The old `src/tup/mempool.c`/`.h` was removed; per-thread `_Thread_local` pools were replaced by jp_alloc's lock-free design.
- jp_alloc's freelist is **64-bit CAS + 3-epoch-ring EBR + per-thread fixed-array cache (N=32 per size class)**. The earlier 128-bit tagged-pointer (cmpxchg16b) implementation was fully replaced by this design; `-mcx16` is no longer required on x86-64 (it was dropped from `Tuprules.tup` and `build.sh`). ABA is prevented by EBR: a popped block cannot reappear at the global freelist head while any thread is still mid-pop. The TLS cache delays returns to the global freelist and batches them into single-atomic-chain retire calls, amortizing EBR bookkeeping; the cache alone would not eliminate ABA — EBR is the correctness layer.
- Thread teardown is leak-free: a `pthread_key` destructor flushes the exiting thread's caches and per-epoch retired batches into a global limbo indexed by `retire_epoch % 3`, which live threads drain lazily while advancing the global epoch.
- The standalone benchmark/correctness test `src/jp_alloc/jp_alloc_bench.c` (compiled with `-DJP_ALLOC_BENCH` and `jp_alloc.c` into a `main()` binary) spawns 300 pthreads doing tup-shaped sized + unsized alloc/free churn and reports ops/sec, p50/p99 latency, and peak RSS. The shell wrapper `test/t5800-jp-alloc-threads.sh` builds release + `-DJP_ALLOC_DEBUG` + 300-thread-stress variants; the debug variant aborts on ABA / double-free / wrong-API / size-mismatch, so reaching "no ABA" output is the regression gate. Note that ASan/TSan are incompatible with the malloc-override and are not exercised by the wrapper.
